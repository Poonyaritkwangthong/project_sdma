...
@section('content')
    @include('layouts.questionnaire_edit.script_edit')
    <style>
        #page-1,
        #page-2,
        #page-3 {
    ...
    <div class="w-[90rem] h-full p-[2rem] border mx-auto rounded-xl shadow-2xl mb-[4rem]">
        <div id="page-1" class="active">
            ...
            @include('layouts.questionnaire_edit.result_input_edit_script')
            <form action="{{ route('general_information.update', $general_information->id) }}" method="POST" enctype="multipart/form-data">
                @csrf <!-- ป้องกัน CSRF (จำเป็นใน Laravel) -->
                @method('PUT') <!-- เพื่อส่งคำขอแบบ PUT -->
                <div class="mb-[2rem] p-[2rem] border-b">
                    @include('layouts.questionnaire_edit.part_01_edit')
                    @include('layouts.questionnaire_edit.part_02_edit')
                </div>
        ...
        <div id="page-2">
            <div class="mb-[2rem] p-[2rem] border-b">
                @include('layouts.questionnaire_edit.part_03_edit')
                @include('layouts.questionnaire_edit.part_04_edit')
                @include('layouts.questionnaire_edit.part_05_edit')
            </div>
        ...
        <div id="page-3">
            <div class="mb-[2rem] p-[2rem] border-b">
                @include('layouts.questionnaire_edit.part_06-08_edit')
                @include('layouts.questionnaire_edit.part_09-10_edit')
            </div>
